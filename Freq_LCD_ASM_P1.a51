...
"Hz" will be shown at the end of the LCD's previous data
...
TIMER_MODE			EQU 15H		; Timer1 Mode 1 Timer 0 Mode 1 (counter)
...
MAIN_STAY:
	JBC Calulation_Flag, MAIN_CALC_JMP	; Monitor the flag
	SJMP MAIN_STAY				; stay here
	
	MAIN_CALC_JMP:				; For resolving the RET jump issue, 
	ACALL CALC
	SJMP MAIN_STAY
;###########################################################################
;###########################################################################
...
SYSINIT:
	...
	MOV TMOD, 	#TIMER_MODE		; Timer 1 mode 1, Timer 0 mode 1 counter mode
...
ADJUSTMENT:					; Here a bug has been fixed if there is zero frequency
	DEC 	R0				; then the next routing would have decremented the R0
	DJNZ	R1, MOVE_AGAIN			; from 0 to FF and it would have displayed till then

	;DEC R1					; old implementation
	;SJMP MOVE_AGAIN			; old implementation
	
	INC R1					; Till here R1 was 0, increment is done so that DISPLAY_DATA
	INC R0					; keeping the R0 pointing at last register R2 it it was zero earlier
	SJMP	DISPLAY_DATA			; Can display one data unit the R0 is currently pointing to
